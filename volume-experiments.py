...
nc = 600    #nodes in x-direction
nr = 600    #nodes in y-direction
dx = 0.025   #horizontal node spacing in meters

# Define constants
h_c = 0.08            		#characteristic debris thickness in meters
D = 0.01   #debris diffusivity constant in meters per day, less than 0.01 m/day for grains smaller than 0.01 m diameter
...
debris_depths = [0.25, 0.5, 1., 2.] 	#pit depth in meters
radii = [0.125, 0.25, 0.5, 1.]    #pit radius in meters

pit = 'yes' #Type 'yes' if the debris patch exists in a pit, otherwise type 'no'

# Create a directory to hold the experiments
dirname = "volume-experiments/"
...
for debris_depth in debris_depths:
    for radius in radii:
        ...
        indx_filename = "volume-experiments_index.txt"

        simFileName = "volume-experiments_"
