...
D = 0.005   #debris diffusivity constant in meters per day, less than 0.01 m/day for grains smaller than 0.01 m diameter
...
b_0 = 0.04       #bare ice melt rate in meters per day
debris_depths = [0.5] 	#pit depth in meters
...
dirname = "time-dependent-experiments-3d/"
...
for debris_depth in debris_depths:

    # Create a text file to hold the parameters from each experiment
    indx_filename = "time-dependent-experiments_3d-index.txt"

    simFileName = "time-dependent-experiments-3d_" + str(debris_depth).replace('.', '') + "m_"
    ...
    for ts in range(tsteps):
        ...
        if ts*dt == 0 or ts*dt == 18 or ts*dt == 36:
            time = (ts+1)*dt
            #z_transects[time] = mg.node_vector_to_raster(z)[nr//2, :]
            #s_transects[time] = copy.deepcopy(mg.node_vector_to_raster(s)[nr//2, :])

            pickle_fname = dirname + "/" + simFileName + "z_time=" + str(time) + ".pkl"
            pf = open(pickle_fname, 'wb')
            pickle.dump(z, pf)
            pickle_fname = dirname + "/" + simFileName + "deb_time=" +str(time) + ".pkl"
            pf = open(pickle_fname, 'wb')
            pickle.dump(h, pf)
            pf.close()
